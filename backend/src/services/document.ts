...
async function buildSalaryCertData(
  employee: any,
  serviceCharge: any,
  tf: Record<string, string> | null,
  ...
  return {
    ...
    employment_date: tf?.employment_date
...
async function buildEmpCertData(
  employee: any,
  tf: Record<string, string> | null,
): Promise<Record<string, string>> {
  return {
    ...
    employment_date: tf?.employment_date
      ? formatDate(new Date(tf.employment_date))
      : formatDate(employee.employment_date),
    last_working_date: tf?.last_working_date
